...
void TMassTable::Init(const char* file)
{
  int n,z,a;
  int counter=0;
  char sym[3];
...
void TMassTable::PrintEntry(int i)
{
...
int TMassTable::Lookup(int n, int z, int a)
{
  int counter=0;
  int returnvalue=0;
...
int TMassTable::Lookup(char *symbol, int a)
{
  char test[3];
...
float TMassTable::GetMassExcess(const char *symbol, int a)
{
  char test[3];
  sprintf(test,"%s",symbol);
  int counter=0;
...
float TMassTable::GetMassExcess(int n=0, int z=0, int a=0)
{
  int counter=0;
  float returnvalue=0;
  if (a==0) a=n+z;
...
const char* TMassTable::GetSymbol(int z=0)
{
  int counter=0;
...
int TMassTable::GetZ(char *symbol)
{
  char test[3];
  sprintf(test,"%s",symbol);
  int counter=0;
...
//added by zmp:
bool TMassTable::GetInitStatus(){//return true for initialized, false otherwise
  return this->initstatus;
};

float TMassTable::GetMassMeV(const char* symbol, int a){//return the mass in MeV/c^2 instead of mass excess - simpler for the end user! (its me im the end user)
